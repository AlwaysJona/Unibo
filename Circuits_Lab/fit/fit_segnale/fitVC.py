...
def ampiezza_capi_condensatore(t, R, V0, w, L, C, b): # y = 

    y = np.cos(w*t + (np.arctan((1-w**2*L*C)/R))-np.pi/2)

    return b*(V0/(w*C))/(np.sqrt(R**2 + (w*L - 1/(w*C))**2))*y
...
P0 = [R,V0,w,L,C,1] 
popt, pcov = curve_fit(ampiezza_capi_condensatore, # function to be fitted (defined above)
                       t, VC, # data 
...
# plot data and fit
fig, ax = plt.subplots()
ax.errorbar(t,VC,yerr=yerr, fmt= '.', ecolor = "red") # uncomment to show also errorbars
ax.plot(t,VC, '.', label = "data", color = 'steelblue', markersize = 1)
ax.plot(t,ampiezza_capi_condensatore(t, *popt), 
         color = 'orange', 
         label = "FIT")
...
ax.set_title("Fit Potenziale ai capi del Condensatore")
ax.set_ylim(-1.5,1.5)
plt.savefig("./fitVC_Adj.png") # --> Per salvare
